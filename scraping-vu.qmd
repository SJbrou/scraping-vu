...
The [https://research.vu.nl/en/courses/?year=2024&ordering=title&descending=false&format=rss&page=](VU%20website) only shows 50 pages of 50 results, while there are \~3500 different courses. Lets querry them orderd on title both ascendingly and descendingly, and find all unique urls.

Note, to run the code, change eval=FALSE to eval=TRUE. Since requesting the \~3k VU course pages takes some time, I stored the results in RDATAfiles that I loaded locally in between sessions. Those are available on github as well.

Lets start by cleaning the worksapce and loading required dependencies

```{r clearworkspace, eval=FALSE}
